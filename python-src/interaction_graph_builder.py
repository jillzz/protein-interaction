...
def build_graph_from_edgelist(path, nodes = None):
    """Build undirected graph from file in edgelist format, 'nodeID nodeID [weight]'
       where weight is optional. nodes is the number of nodes"""
    G = nx.Graph()
    if nodes:
